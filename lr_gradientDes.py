...
plt.tight_layout()
#sort them and find the most corrrelated ones
#mapped = np.vstack((corrPrcCoef, corrFeat))
#sortedMapped = sorted(mapped.T, key=lambda x:x[0])
#for i in sortedMapped[::-1]:
#    print('Feature:', i[1], ': ', i[0])
...
##validation
#valLRate = 6*10**-7
#valValues = np.arange(10*10**-8,15*10**-7, 10**-7)
#avrErrLog = []
#epoch = 20
#for j in valValues:
#    print('For N =' , j)
...
#plt.title('Validation percentage error by trained epocs')
#plt.ylabel('%')
#plt.xlabel('Epocs')
...
for ep in range(epochL):
    ...
    for i in range(len(trainData)):
        ...
        dWeights =  err * trainData[i]#np.sum( err * trainData, axis  = 0)[np.newaxis].T
...
plt.ticklabel_format(axis='x',style='sci',scilimits=(0,3))
plt.title('Training percentage error through epocs')
plt.ylabel('%')
plt.xlabel('Epocs')
